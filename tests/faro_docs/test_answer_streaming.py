...
class TestNoCorrectiveRetry:
    """A "you didn't call a tool, do it now" corrective round was tried and
    reverted. It measured better on a small local test model (turned wrong
    answers into right ones on a 50-page catalogue) but made real usage on
    the actual office model worse: the directive "STOP, call a tool now"
    language appears to push a stronger model toward writing a pseudocode
    imitation of a tool call (`<tool_code>print(...)`) instead of using
    Ollama's real tool-calling mechanism, and doubles generation time for
    every affected question, worsening the separate connection-drop issue.
    Reverted 2026-09-23 on direct report from real office testing. Content
    streams live immediately again, with no withholding or retry, exactly
    as it did before that experiment."""

    def test_a_counting_question_answered_without_a_tool_streams_immediately(self, monkeypatch):
        client = _RoundAwareFakeClient([
        ...
        assert client.call_count == 1, "no retry round, even though no tool was called"
        assert "There are about 12." in text

    def test_a_plain_question_is_not_held_back(self, monkeypatch):
        client = _RoundAwareFakeClient([
